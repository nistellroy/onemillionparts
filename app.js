...
app.post("/index", (req, send) => {
    const go = async () => {
        const res = await axios({
            ...
            data: "user_name=Sharof&pass_word=shar-@!_ov&remember_me=1" //form data login and password site htt://onemillionparts.com/app.php
        });
        const getHeaders = await res.headers['set-cookie'].toString();

        const response = await axios({
            method: "GET",
            url: `https://onemillionparts.com/controller/buying/read.php?_dc=1667349325477&StorageSearchingCase=1&Storage=${!isNaN(parseInt(req.body.Count)) ? "mf" + req.body.Count : req.body.Count}&page=1&start=0&limit=50`,
            headers: {
                'Cookie': `${getHeaders}`
            },
        });
